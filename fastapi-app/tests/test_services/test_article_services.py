...
@pytest.mark.asyncio
async def test_mock_service_article_true(
    mock_logic_db_repository, mock_article_db_repository
):
    ...
    dto = ArticleCreateDTO(title="title", category="category", content="contentcontent")
...
@pytest.mark.asyncio()
async def test_mock_service_article_false(
    mock_logic_db_repository, mock_article_db_repository
):
    base_service = ArticleService(
        base_repository=mock_article_db_repository,
